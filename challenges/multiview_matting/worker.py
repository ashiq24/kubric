...
# new dome object
##
# --- Parameters for the new dome
dome_radius = 2.0  # Adjust radius for desired dome size
dome_height = 1.0   # Height of the dome

# Create a sphere to represent the dome
dome_sphere = kb.Sphere(name="dome_sphere", 
                         scale=(dome_radius, dome_radius, dome_height),
                         position=(0, 0, dome_height / 2), 
                         static=True, background=True)

# Define a material for the dome
dome_material = kb.PrincipledBSDFMaterial(
    color=kb.Color(r=1.0, g=1.0, b=1.0),  # White color for diffuse lighting
    metallic=0.0,  # Disable metallic reflection
    roughness=0.5,  # Adjust roughness for desired scattering
    emission=kb.Color(r=0.5, g=0.5, b=0.5),  # Emission for ambient light
)

# Assign the material to the dome
dome_sphere.material = dome_material

# Add the dome to the scene
scene.add(dome_sphere)
